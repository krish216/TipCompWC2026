...
function SettingsTab({ comp, tier, domain, minAge, maxTribeSize, requiresFee, entryFee, currentUserId, tipsters, onUpdate }: { comp: any; tier: string; domain: string | null; minAge: number | null; maxTribeSize: number; requiresFee: boolean; entryFee: number | null; currentUserId: string; tipsters: Tipster[]; onUpdate: (k: string, v: any) => void }) {
  ...
  const [ocPrizeDesc,     setOcPrizeDesc]     = useState<string>(comp?.prize_description ?? '')
  const [ocMemberCap,     setOcMemberCap]     = useState<string>(comp?.member_cap ? String(comp.member_cap) : '')
  ...
  useEffect(() => {
    if (!comp?.tournament_id) return
    fetch(`/api/tournament-teams?tournament_id=${comp.tournament_id}`)
      .then(r => r.json()).then(d => setTeamsList(d.teams ?? [])).catch(() => {})
  }, [comp?.tournament_id])

  const saveOpenCompSettings = async () => {
    setOcSaving(true); setOcError(null)
    ...
    setOcSaving(false)
    if (!res.ok) { const d = await res.json(); setOcError(d.error ?? 'Failed to save'); return }
    onUpdate('visibility', ocVisibility)
  }
  ...
  const saveLogo = async (explicitUrl?: string) => {
    const urlToSave = (explicitUrl ?? logoUrl).trim() || null
    const res = await fetch('/api/comps/create', {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ comp_id: comp.id, logo_url: urlToSave }),
    })
    if (res.ok) { onUpdate('logo_url', urlToSave); toast.success('Logo updated') }
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const path = `comps/${comp.id}/logo.${ext}`
  ...
  return (
    <div className="space-y-0">
      ...
      {[
        ...
        { title: 'Comp logo', sub: 'Shown in your comp header', id: 'comp-logo-section', content: (
          <div className="p-4 space-y-3">
        ...
        { title: 'Comp settings', sub: 'Control visibility, audience and prize for this comp', content: (
          <div className="p-4 space-y-4">
            {/* Visibility */}
            <div>
              <p className="text-xs font-medium text-gray-700 mb-1.5">Who can join?</p>
              <div className="grid grid-cols-2 gap-2">
                {([
                  { val: 'private', icon: '🔒', label: 'Private', sub: 'Invite only' },
                  { val: 'open',    icon: '🌐', label: 'Open',    sub: 'Anyone can join' },
                ] as const).map(opt => (
                  <button key={opt.val} type="button" onClick={() => setOcVisibility(opt.val)}
                    className={`flex items-center gap-2 py-2 px-3 rounded-xl border-2 transition-colors
                      ${ocVisibility === opt.val ? 'border-gray-800 bg-gray-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
            ...
            <button onClick={saveOpenCompSettings} disabled={ocSaving || (ocVisibility === 'open' && ocCategory === 'team_fans' && !ocTeamAffil)}
              ...
              {ocSaving && <Spinner className="w-3 h-3 text-white" />}
              Save
            </button>
          </div>
        )},
    ...
    <div className="max-w-2xl mx-auto px-4 py-4">
      ...
      <div className="bg-sky-50 border border-sky-100 rounded-2xl px-4 pt-4 pb-0 mb-5 overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-3">
          {comp.logo_url
            ? <img src={comp.logo_url} alt={comp.name} className="w-10 h-10 rounded-xl object-cover border border-sky-200" />
            : <div className="w-10 h-10 rounded-xl bg-sky-700 flex items-center justify-center text-white text-base font-black flex-shrink-0">{comp.name?.[0] ?? '?'}</div>
          }
          <div className="flex-1 min-w-0">
          ...
          <div className="relative flex-shrink-0" ref={kebabRef}>
            <button onClick={() => setShowKebab(v => !v)}
              className="w-8 h-8 flex items-center justify-center rounded-xl text-sky-400 hover:text-sky-700 hover:bg-sky-100 transition-colors text-lg font-bold leading-none">
              ···
            </button>
            {showKebab && (
        ...
        {comp.invite_code && (
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <div className="flex items-center gap-2 px-3 py-2 bg-white border border-sky-200 rounded-xl">
              <span className="text-xs text-sky-500 font-semibold">Code</span>
              <span className="font-mono font-black tracking-widest text-sky-900 text-sm select-all">{comp.invite_code}</span>
              <button onClick={async () => { await navigator.clipboard.writeText(comp.invite_code); toast.success('Code copied!') }}
                className="text-[11px] text-sky-400 hover:text-sky-700 font-semibold transition-colors">
                copy
              </button>
            </div>
            <button
