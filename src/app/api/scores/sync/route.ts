...
export async function GET(request: NextRequest) {
  ...
  // Auto-post the tribe Round Debrief for any knockout round that's now fully
  // scored (idempotent). Best-effort — never blocks results.
  ...
  if (!pending?.length) return NextResponse.json({ updated: 0, schedule, reminders, debriefs, message: 'No pending fixtures' })
  ...
  return NextResponse.json({ updated, checked: pending.length, schedule, reminders, debriefs, skipped, source: 'espn', timestamp: now.toISOString() })
}
