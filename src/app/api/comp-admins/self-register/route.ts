...
export async function POST(request: NextRequest) {
  ...
  // Auto-enrol in the comp's default tribe, or the sole tribe if only one exists
  ...
  if (targetTribe) {
    const compTribeIds = tribes.map(t => t.id)
    ...
    if (!alreadyInTribe) {
      await (adminClient.from('tribe_members') as any)
        .upsert({ user_id: user.id, tribe_id: targetTribe.id }, { onConflict: 'user_id,tribe_id', ignoreDuplicates: true })
      tribeAssigned = true
    } else {
