...
async function loadScoringConfig() {
  try {
    ...
    if (!tournament) return getDefaultScoringConfig()
    ...
    if (!rounds?.length) return getDefaultScoringConfig()
    return buildScoringConfig(rounds as any)
  } catch {
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="grid grid-cols-[1fr_60px_52px_1fr] text-[10px] font-semibold text-gray-500 uppercase tracking-wide px-4 py-2 bg-gray-50 border-b border-gray-200">
            <span>Round</span>
            <div className="text-center leading-tight">Pick type</div>
            <div className="text-right leading-tight"><span className="text-green-700">Base</span><br/>pts</div>
            <div className="text-right leading-tight"><span className="text-amber-600">⚡</span> Bonus pts</div>
          </div>
          {SCORING_ROWS.map(({ rid, label, badge, highlight }) => {
            ...
            const marginBonus  = sc?.margin_bonus  ?? 0
            const penBonus     = sc?.pen_bonus     ?? 0
            const favBonus     = sc?.fav_team_2x   ?? false
            const hasBonus     = exactBonus > 0 || marginBonus > 0 || penBonus > 0 || favBonus
            return (
              <div key={rid} className={`grid grid-cols-[1fr_60px_52px_1fr] px-4 py-3 border-b border-gray-100 last:border-0 ${highlight ? 'bg-amber-50' : ''}`}>
                ...
                <div className="text-right text-[11px] space-y-0.5">
                  ...
                  {marginBonus > 0 && <div className="text-orange-500 font-semibold">+{marginBonus} correct margin</div>}
        ...
        <div className="mt-3 flex gap-3 flex-wrap text-xs text-gray-500">
          ...
          <span><span className="font-medium text-purple-700">+Exact</span> — extra pts for the precise scoreline</span>
          <span><span className="font-medium text-orange-500">+Margin</span> — extra pts for correct goal difference (not exact)</span>
          <span><span className="font-medium text-gray-500">✗ Wrong result</span> — 0 pts</span>
        </div>
        <p className="text-xs text-gray-400 mt-2">Semi-finals, 3rd place play-off and Final require a score prediction. All other rounds use 1/X/2 outcome only.</p>
      </section>

      {/* Margin bonus explainer */}
      <section className="mb-8">
        <h2 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wide">Correct margin bonus 📐</h2>
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
          <p className="text-sm text-gray-700 mb-3">
            On score-prediction rounds (Semi-finals, 3rd Place, Final): if you predict the right result (win or draw)
            but not the exact scoreline, you still earn a <strong className="text-orange-600">+{scoringConfig.rounds['sf']?.margin_bonus ?? 5} margin bonus</strong> if
            your predicted goal difference matches the actual goal difference.
          </p>
          <div className="space-y-2 mb-3">
            {[
              { pred: '3–2', result: '1–0', margin: '1 = 1', awarded: true,  note: 'Correct result (Home win) · margins match → margin bonus ✓' },
              { pred: '2–0', result: '3–0', margin: '2 = 3', awarded: false, note: 'Correct result (Home win) · margins differ → no bonus' },
              { pred: '1–1', result: '2–2', margin: '0 = 0', awarded: true,  note: 'Correct result (Draw) · margins always 0 → margin bonus ✓' },
              { pred: '1–0', result: '1–0', margin: '1 = 1', awarded: false, note: 'Exact score → exact bonus applies instead, not margin bonus' },
            ].map(ex => (
              <div key={ex.pred + ex.result} className="flex items-start gap-3 bg-white rounded-lg border border-orange-100 px-3 py-2">
                <span className="text-base flex-shrink-0">{ex.awarded ? '✅' : '➖'}</span>
                <div>
                  <p className="text-xs font-medium text-gray-800">
                    Predict <strong>{ex.pred}</strong> · Result <strong>{ex.result}</strong> · Margins: {ex.margin}
                  </p>
          ...
          <p className="text-xs text-gray-500">
            The exact bonus and margin bonus are mutually exclusive — you cannot earn both on the same prediction.
            Penalty bonus and margin bonus can stack if the conditions for both are met.
          </p>
        </div>
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <p className="text-sm text-gray-700 mb-3">
            In all knockout rounds (Round of 32 onwards), matches cannot end in a draw. A penalty shootout winner must be picked
            for Semi-finals, 3rd Place play-off and Final when predicting a draw score.
          </p>
          <div className="space-y-2 mb-3">
            {[
              { scenario: 'Predict 1–1 · correct pen winner',   result: 'Correct result + exact bonus + pen bonus', pts: true  },
              { scenario: 'Predict 1–1 · wrong pen winner',     result: 'Correct result + exact bonus only',        pts: false },
              { scenario: 'Predict 2–2 · actual 1–1 · correct pen winner', result: 'Correct result + margin bonus + pen bonus', pts: true },
          ...
          <p className="text-xs text-gray-500">
            The penalty pick appears automatically when you enter an equal score for any knockout fixture (Round of 16 onwards).
          </p>
        </div>
      </section>
