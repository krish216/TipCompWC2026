    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {SCORING_ROWS.map(({ rid, label, badge, highlight }, i) => {
            ...
            const hasPenBonus = ['r16','qf','sf','tp','f'].includes(rid)
            return (
              <div key={rid} className={`grid grid-cols-[1fr_70px_64px_80px] px-4 py-3 border-b border-gray-100 last:border-0 ${highlight ? 'bg-amber-50' : ''}`}>
                ...
                <div className="text-center">
                  <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded-full ${isExact ? 'bg-purple-100 text-purple-700' : 'bg-blue-50 text-blue-600'}`}>
                    {isExact ? 'Score' : '1/X/2'}
                  </span>
                </div>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Group stage bonus score',    normal: 5, bonus: 10 },
              { label: 'Round of 32 correct result', normal: 5, bonus: 10 },
              { label: 'Round of 32 bonus score',    normal: 8, bonus: 16 },
            ].map(ex => (
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          {[
            { icon:'🔗', text:'Join a tribe using your comp\'s invite code — or enter a tribe code directly' },
