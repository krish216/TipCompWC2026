...
interface Tribe  { id: string; name: string; invite_code: string; member_count?: number }
...
function CreateTribeForm({ orgId, onCreated }: { orgId: string; onCreated: (t: Tribe) => void }) {
  ...
  const submit = async () => {
    ...
    else { toast.success(`Tribe "${data.name}" created`); setName(''); onCreated(data) }
  }
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <button
            onClick={copyCode}
            className={clsx(
              'flex items-center gap-1.5 mt-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono font-medium border transition-colors',
              copied
...
export default function OrgAdminPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!adminData.is_org_admin) {
        setIsOrgAdmin(false); setLoading(false); return
      }
  ...
  if (!isOrgAdmin) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center">
      ...
      <p className="text-sm text-gray-500 mb-6">
        You don't have organisation admin access. Contact your tournament administrator to be granted this role.
      </p>
    ...
    <div className="max-w-2xl mx-auto px-4 py-4">
      ...
      <div className="mb-5 bg-blue-50 border border-blue-200 rounded-xl p-4">
        ...
        <div className="text-xs text-blue-700 space-y-1">
          ...
          <p>✅ View all members in your organisation</p>
      ...
      {tribes.length === 0 ? (
        <EmptyState
          title="No tribes yet"
          description="Create the first tribe for your organisation using the form above."
        />
      ) : (
