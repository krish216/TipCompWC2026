...
// CTA for the $4.95 one-time Tipster Pro pass (per tournament) — ad-free now, with
  ...
  if (variant === 'link') {
    return (
      <button onClick={go} disabled={loading}
        ...
        {loading ? 'Redirecting…' : 'Remove ads · $4.95'}
      </button>
    )
  }
  ...
  if (variant === 'menu') {
    return (
      <button onClick={go} disabled={loading}
        ...
        <span>🚫</span> {loading ? 'Redirecting…' : 'Go ad-free · $4.95'}
      </button>
    )
  }
